...
def weighted_knn_transfer(
    ...
    # this is not the regular guassian kernel
    # sigma is first transfromed to 2 / sigma
    # the intention is probably:
    #   - in dense region, knn distances have low std, so broad similarity range
    #   - in sparse region, knn distances have high std, so narrower similarity range
    #   - in dense region, consider label tranfer across a larger range of candidate (in a soft sense)
    #   - in sparse region, consider label transfer only from closest candidates (in a soft sense)
    # make sense for label transfer to restrict search range for average label
